#! /bin/bash
WLANNM=$(cat /root/neutrality/wlanmon.txt)
export WLANNM
#-----------------FUNCTIONS-----------------
function defaults_neutralityh1
{
#path for neutrality
...
then
	...
	gnome-terminal --geometry 70x10+0+0 -- neutralityh2
	HANDFORDEAUTH="/root/handshakes/$HANDF-01.csv"
	export HANDFORDEAUTH
	airodump-ng --bssid $TARGB -c $TARGC -w /root/handshakes/$HANDF $WLANNM # & jobs -p > /root/neutrality/dpidd.txt
	...
	if [[ -f "/root/neutrality/dpid*.txt" ]]
	then
		rm /root/neutrality/dpid*.txt
	fi
	validhandshake
	exec bash "$0"
else
...
{
		rm -f /root/neutrality/tempairodump-01.cap
		rm -f /root/neutrality/tempairodump-01.csv
		rm -f /root/neutrality/tempairodump-01.kismet.netxml
		rm -f /root/neutrality/tempairodump-01.kismet.csv
		rm -f /root/neutrality/dpid*.txt
}
function autoapselect
{
file="/root/neutrality/tempairodump-01.csv"
if [[ -f "$file" ]]
then
...
do
	...
	then
		...
		then
			FJC=0
			JC=1
			airodump-ng $WLANNM -w /root/neutrality/tempairodump
		...
		then
			...
			FJC=0
			JC=1
			airodump-ng -c $CHANNELSL $WLANNM -w /root/neutrality/tempairodump
		fi
	fi
		ap_sort_and_choose
		if [[ "$APCHOOSE" = "r" ]]
